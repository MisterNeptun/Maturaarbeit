def spiel_2(player1,player2):
    ...
    for i in range(10000):
        player1.wurf(verkaufteimobilien, karten1, karten2, karten1zumauffuellen, karten2zumauffuellen,nochnichtgekaufteimmobilien)
        anzahlgebauterhaeuserimzug=0
        if player1.wert2*10000>player1.geld:
            player1.gekauft.sort(key=lambda x: x.startwert, reverse=player1.wert3)
            for i in range(len(player1.gekauft)):
                if player1.gekauft[i].bauenueberpruefen(player2,verkaufteimobilien)==True:
                    anzahlgebauterhaeuserimzug=anzahlgebauterhaeuserimzug+1
                player1.gekauft[i].bauen(player1,verkaufteimobilien)
                if anzahlgebauterhaeuserimzug>0 and player1.geld<5000*player1.wert4:
                    break

        #print(player1.geld, player2.geld)
        if player1.geld<0:
            while True:
                player1.gekauft.sort(key=lambda x: x.startwert, reverse=player1.wert5)
                x=0
                for i in range(len(player1.gekauft)):
        ...
        player2.wurf(verkaufteimobilien, karten1, karten2, karten1zumauffuellen, karten2zumauffuellen,nochnichtgekaufteimmobilien)
        anzahlgebauterhaeuserimzug=0
        if player2.wert2 * 10000 > player2.geld:
            ...
            for i in range(len(player2.gekauft)):
                if player2.gekauft[i].bauenueberpruefen(player2,verkaufteimobilien)== True:
                    anzahlgebauterhaeuserimzug = anzahlgebauterhaeuserimzug + 1
                player2.gekauft[i].bauen(player2,verkaufteimobilien)
                if anzahlgebauterhaeuserimzug>0 and player2.geld<5000*player2.wert4:
                    break

        #print(player1.geld, player2.geld)
        if player2.geld<0:
            while True:
                player2.gekauft.sort(key=lambda x: x.startwert, reverse=player2.wert5)
                x=0
                for i in range(len(player2.gekauft)):
